...
***
"""
up_to_here = '\\'.join(os.path.abspath(__file__).split('\\')[:-1]).replace('\\','/')
print(up_to_here)
blender_version = 'blender2.78'
...
def render_image(key_light_jitter=[1, 2, 3, 4, 5],
    ...
    collected_locals = dict(locals().items())
    if clean_before:
...
if USE_GPU == 1:
    ...
    render_image(key_light_jitter=[1],
                 fill_light_jitter=[1],
                 back_light_jitter=[1],
                 camera_jitter=[1],
                 num_objects=[1],
                 per_image_shapes=[[None]],
                 per_image_colors=[[None]],
                 per_image_materials=[[None]],
                 per_image_sizes=[[None]],
                 per_image_x=[[1.9]],
                 per_image_y=[[2.5]],
                 per_image_theta=[[0]],
                 workers=1,
...
def main(agent='random', scale=0.1, noise='gaussian', img_offset=0, max_episodes=1, batch_size=2,
         max_complete_images=7_000):
...
#     args = parser.parse_args()
    # main(str(args.agent), float(args.scale), str(args.noise), int(args.img_offset), int(args.max_episodes),
    #      int(args.batch_size))
