...
def main(use_gpu=1, split='val', agent='random', scale=0.1, noise='gaussian', img_offset=0, max_episodes=1,
         batch_size=2, max_complete_images=7_000):
    USE_GPU = use_gpu
    SPLIT = split
    ...
    ***
    ...
    def scene_creation(shape_list=[None], color_list=[None], material_list=[None], size_list=[None], x_list=[None],
                       y_list=[None], theta_list=[None], multiply=1):
        """
        ...
        for sh, c, m, si, x, y, t in zip(shape_list * multiply, color_list * multiply, material_list * multiply,
                                         size_list * multiply, x_list * multiply, y_list * multiply,
                                         theta_list * multiply):
    ...
    ***
    ...
    if USE_GPU == 1:
        ...
        render_image([1], [1], [1], [1], [1], [[None]], [[None]], [[None]], [[None]], [[1.9]], [[2.5]], [[0]], 1,
                     'warmup', 0)
        ...
        def __init__(self, padding_number=-10, scenes=f'{up_to_here}/official_val/CLEVR_val_scenes.json'):
            ...
            if scenes is not None:
                try:
                    ...
                    print(
                        "Official Validation Scenes not found at: {up_to_here}/official_val/CLEVR_val_scenes.json ...")
                    scenes = None
            self.scenes = scenes
    ...
    ***
    ...
    generator = make_questions(word_replace_dict={'True': 'yes', 'False': 'no'},
                               output_questions_file=f'{up_to_here}/questions/CLEVR_{SPLIT}_questions.json')
