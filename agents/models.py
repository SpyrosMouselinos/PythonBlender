...
class InformedRandomAgent(nn.Module):
    def __init__(self, scenes=None, persist_contiguous=False, padding_number=-10):
        super(InformedRandomAgent, self).__init__()
        self.persist_contiguous = persist_contiguous
        ...
        if scenes is not None:
            try:
                ...
                print(f"Official Validation Scenes not found at: {self.scenes} ...")
                scenes = None
        self.scenes = scenes
    ...
    def retrieve_configs(self, indexes: list):
        n_objects = []
        xs = []
        ys = []
        thetas = []
    ...
    def forward(self, objects=-1, start_idx=0, batch_size=1, indexes=None, randomize_idx=False):
        if indexes is not None:
            assert self.scenes is not None

        assert 0 <= start_idx <= 14990
        if randomize_idx:
            start_idx = np.random.randint(0, 14990)
        ...
        if self.scenes is None:
            ...
            (per_image_objects, per_image_shapes, per_image_colors, per_image_materials, per_image_sizes), (
                per_image_x, per_image_y, per_image_theta) = self.retrieve_configs(indexes=indexes)
