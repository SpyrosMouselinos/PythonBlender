...
def main(args):
    ...
    # TODO(Spyros) Make sure that the number of items provided matches the number of images to create
    assert len(args.key_light_jitter) == args.num_images
...
def render_scene(args,
    ...
    def rand(input_L, old_behaviour=False):
        "This is the overloaded deterministic version of rand"
        if old_behaviour:
            return 2.0 * input_L * (random.random() - 0.5)  # Old Effective Range: [-1 * L, +1 * L]
        else:
            return input_L  # New Effective Range: [-inf, +inf]
